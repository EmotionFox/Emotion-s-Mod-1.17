...
public class BasicBoat extends Item
{
	private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
	private final EntityBoat.EmoType type;

	public BasicBoat(EntityBoat.EmoType typeIn, Item.Properties properties)
	{
		super(properties);
		this.type = typeIn;
	}
	...
	public InteractionResultHolder<ItemStack> use(Level p_40622_, Player p_40623_, InteractionHand p_40624_)
	{
		...
		} else
		{
			...
			{
				EntityBoat boat = new EntityBoat(p_40622_, hitresult.getLocation().x, hitresult.getLocation().y, hitresult.getLocation().z);
				boat.setEmoType(this.type);
				boat.setYRot(p_40623_.getYRot());
				if (!p_40622_.noCollision(boat, boat.getBoundingBox().inflate(-0.1D)))
				{
